...
from solver.generate_sim import GenerateSimSF
...
if __name__ == '__main__':
    ...
    # sd_eps = 0.1  # standard deviation of the eps Matern covariance
    ...
    # plt.scatter(range(m), eps, marker=".", color='salmon')
    # plt.show()

    # # plot b with and without errors
    # plt.scatter(range(m), b - eps, marker=".")
    # plt.gca().set_prop_cycle(None)
    # plt.scatter(range(m), b, marker=".", color='salmon')
    # plt.show()
    ...
    MSEy_std2 = np.mean(LA.norm(resy, axis=0) ** 2 / LA.norm(b, axis=0) ** 2)
    MSEy_std = np.mean(LA.norm(resy, axis=0) / LA.norm(b, axis=0))
    ...
    # for i in range(not0):
    #     if indx[i]:
    #         plt.plot(grid, out_SF.x_curves[ind_curve, :].T, '--', color='salmon')
    #         ind_curve += 1
    ...
    #         plt.plot(grid, out_SF.x_curves[i, :].T, '--', color='salmon')
    #         plt.show()
